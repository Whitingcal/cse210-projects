...
namespace Develop05 
{
    public class User 
    {
        ...
        {
            Console.WriteLine("Select the goal you would like to check off:");
            DisplayGoals();
            Console.Write("Goal: ");
        ...
        public void CreateGoal() 
        {
            ...
            {
                case "1":
                    _goals.Add(new Simple(name, description, pointsPer));
                    break;
                case "2":
                    _goals.Add(new Eternal(name, description, pointsPer));
                    break;
                case "3":
                    Console.WriteLine("Times to complete the goal: ");
                    int timesToComplete = Convert.ToInt32(Console.ReadLine());
                    _goals.Add(new Checklist(name, description, pointsPer, timesToComplete));
                    break;
            }
        }
        ...
        private void ReadFromFile() 
        {
            foreach (string line in File.ReadLines(_filename)) 
            {
                ...
                {
                    case "SimpleGoal:":
                                     _goals.Add(new Simple(contents[1], contents[2], Convert.ToInt32(contents[3]), !(contents[4]=="0")));
                                     break;
                    case "EternalGoal:":
                                      _goals.Add(new Eternal(contents[1], contents[2], Convert.ToInt32(contents[3]), Convert.ToInt32(contents[4])));
                                      break;
                    case "ChecklistGoal":
                                      _goals.Add(new Checklist(contents[1], contents[2], Convert.ToInt32(contents[3]), Convert.ToInt32(contents[4]), Convert.ToInt32(contents[5])));
                                      break;
                }
            }
        }
